...
import { registerUser, requestOtp, verifyOtp, loginUser, getUsage } from './services/api';
...
const App: React.FC = () => {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage('Connection failed. Ensure hotspot allows bridge domains.');
      runDiagnostics();
    } finally {
      setIsSubmitting(false);
    }
  ...
  const redirectToPayfast = (amount: number, itemName: string) => {
    ...
    const currentProtocol = window.location.protocol === 'http:' ? 'http:' : 'https:';
    ...
    const data: Record<string, string> = {
      ...
      notify_url: 'https://device.onetel.co.za/api/v1/radius/organization/umoja/account/payment/notify/',
      ...
      cell_number: (formData.phone_number || loginData.username || '0000000000').substring(0, 100),
  ...
  const WALLED_GARDEN = "tmanscript.github.io,device.onetel.co.za,api.allorigins.win,corsproxy.io,api.codetabs.com,esm.sh,cdn.tailwindcss.com,fonts.googleapis.com,fonts.gstatic.com,sandbox.payfast.co.za,umoja.network.coova.org";
  ...
  const renderContent = () => {
        ...
        <div className="max-w-2xl w-full bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-pink-100 animate-in zoom-in duration-300">
          <div className="p-8 sm:p-12">
            ...
            <div className="text-center mb-10">
              <h2 className="text-3xl font-black text-gray-900 mb-2">Select a Data Plan</h2>
              <p className="text-gray-500">Fast, reliable data for all your internet needs.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                ...
                <p className="text-pink-100/70 text-sm mb-4">Valid for 30 Days</p>
        ...
        <div className="max-w-4xl w-full grid grid-cols-1 lg:grid-cols-2 bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-pink-100 animate-in fade-in duration-500">
          <div className="hidden lg:flex flex-col justify-between p-12 bg-pink-500 text-white relative overflow-hidden">
            ...
            <div className="relative z-10 bg-black/10 backdrop-blur-md rounded-2xl p-4 border border-white/10">
              <p className="text-[10px] font-black uppercase tracking-widest mb-3 text-pink-100">Hotspot Status</p>
              <div className="grid grid-cols-2 gap-2">
                {diagnostics.map((d) => (
                  <div key={d.domain} className="flex items-center gap-2 text-[9px] font-bold">
                    <div className={`w-2 h-2 rounded-full ${d.status === 'ok' ? 'bg-green-400' : 'bg-red-400 animate-pulse'}`} />
          ...
          <div className="p-8 overflow-y-auto max-h-[85vh]">
            ...
            <form onSubmit={handleRegistrationSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <Input label="First Name" name="first_name" value={formData.first_name} onChange={handleInputChange} placeholder="John" icon={<User className="w-4 h-4" />} required />
                <Input label="Last Name" name="last_name" value={formData.last_name} onChange={handleInputChange} placeholder="Doe" icon={<User className="w-4 h-4" />} required />
              </div>
              ...
              <Input label="Email Address" name="email" type="email" value={formData.email} onChange={handleInputChange} placeholder="john@example.com" icon={<Mail className="w-4 h-4" />} required />
              <div className="grid grid-cols-2 gap-4">
      ...
      return (
        <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl p-8 border border-pink-100 animate-in zoom-in duration-300">
          ...
          <form onSubmit={handleOtpSubmit} className="space-y-6">
            <Input label="Verification Code" name="otp" value={otpCode} onChange={(e) => setOtpCode(e.target.value)} placeholder="000000" className="text-center text-2xl tracking-[0.5em] font-black" required />
    ...
    if (step === 'SUCCESS' || (step === 'USAGE_INFO' && usageData?.hasData)) {
      return (
        <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-8 text-center border-t-8 border-pink-500 animate-in zoom-in">
          ...
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Authenticated!</h2>
          {usageData && (
            <div className="mb-6 p-4 bg-pink-50 rounded-2xl border border-pink-100">
              <p className="text-pink-600 font-bold text-lg">{usageData.remainingMB} MB Remaining</p>
            </div>
          )}
          <button onClick={connectToRouter} className="w-full py-4 bg-pink-500 hover:bg-pink-600 text-white font-bold rounded-2xl shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2">
    ...
    if (step === 'USAGE_INFO' && !usageData?.hasData) {
      return (
        <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-8 text-center border-t-8 border-orange-500 animate-in zoom-in">
          ...
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Data Depleted</h2>
          <button onClick={() => setStep('BUY_DATA')} className="w-full py-4 bg-orange-500 text-white font-bold rounded-2xl shadow-lg flex items-center justify-center gap-2 active:scale-95">
    ...
    return (
      <div className="max-w-4xl w-full grid grid-cols-1 lg:grid-cols-2 bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-pink-100">
        <div className="hidden lg:flex flex-col justify-between p-12 bg-pink-500 text-white relative overflow-hidden">
          <h2 className="text-4xl font-bold leading-tight mb-6">High Speed WiFi</h2>
          <div className="relative z-10 bg-black/10 backdrop-blur-md rounded-2xl p-4 border border-white/10">
            <p className="text-[10px] font-black uppercase tracking-widest mb-3 text-pink-100">System Health</p>
            <div className="grid grid-cols-2 gap-2">
              {diagnostics.map((d) => (
                <div key={d.domain} className="flex items-center gap-2 text-[9px] font-bold">
                  <div className={`w-2 h-2 rounded-full ${d.status === 'ok' ? 'bg-green-400' : 'bg-red-400 animate-pulse'}`} />
        ...
        <div className="p-8 sm:p-12 flex flex-col justify-center bg-white">
          <h3 className="text-2xl font-bold mb-8 text-gray-900">Login to Onetel</h3>
          ...
          <button onClick={() => setStep('REGISTRATION')} className="w-full mt-6 text-pink-500 font-bold text-xs uppercase tracking-widest hover:underline">Create New Account</button>
        </div>
      </div>
    );
  };
        ...
        <div className="mt-8 max-w-xl w-full bg-white border-2 border-pink-100 rounded-[2rem] p-6 shadow-xl animate-in slide-in-from-bottom-8">
           <div className="flex items-center justify-between mb-4">
              <h4 className="text-[10px] font-black text-gray-800 uppercase tracking-widest flex items-center gap-2">
                <Info className="w-4 h-4 text-pink-500" /> Walled Garden Requirement
              </h4>
              <button onClick={() => setShowHelper(false)} className="text-gray-400 font-bold text-[9px] uppercase">Dismiss</button>
           </div>
           <p className="text-[10px] text-gray-500 mb-3 font-medium">Add these to your router's <b>uamallowed</b> list or connection will fail:</p>
